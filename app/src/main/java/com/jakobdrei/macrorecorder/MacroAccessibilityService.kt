...
class MacroAccessibilityService : AccessibilityService() {
    ...
    override fun onDestroy() { instance = null; stopPlayback() }
    ...
    fun startPlayback() {
        ...
        val endDelay = actions.last().relativeMs + actions.last().duration + 300L
        handler.postDelayed({ if (!stopRequested) MacroManager.onPlaybackFinished(this) }, endDelay)
    }
    ...
    private fun dispatchTap(x: Float, y: Float, dur: Long) {
        ...
        dispatchGesture(GestureDescription.Builder()
            .addStroke(GestureDescription.StrokeDescription(p, 0, dur.coerceIn(1L, 9999L)))
            .build(), null, null)
    }
